// Co-Work API Client
// Connects to Co-Work workspace backend
...
export type SearchFilterType = 'all' | 'notes' | 'emails' | 'tasks' | 'events';
...
export const searchAPI = {
  ...
  async searchNotes(query: string, limit = 10): Promise<SearchResponse> {
    return searchAPI.search(query, 'notes', limit);
  },

  async searchEmails(query: string, limit = 10): Promise<SearchResponse> {
    return searchAPI.search(query, 'emails', limit);
  },

  async searchTasks(query: string, limit = 10): Promise<SearchResponse> {
    return searchAPI.search(query, 'tasks', limit);
  },

  async searchEvents(query: string, limit = 10): Promise<SearchResponse> {
    return searchAPI.search(query, 'events', limit);
  },

  async getHealth(): Promise<{ available: boolean; status: string; message: string; cluster_name?: string; version?: string }> {
